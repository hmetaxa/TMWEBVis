(function () {
    ...
    exports.conference = "SELECT BatchId as xaxis, EntityTopicDistribution.TopicId as topicId, NormWeight as weight, TopicDescription.Title as title, TopicDescription.VisibilityIndex, EntityId as ids FROM EntityTopicDistribution INNER JOIN TopicDescription ON TopicDescription.TopicId = EntityTopicDistribution.TopicId  EntityType = 'ConferenceTrend' AND TopicDescription.ExperimentId=EntityTopicDistribution.ExperimentId AND VisibilityIndex=5 WHERE  EntityTopicDistribution.ExperimentId = ?";
